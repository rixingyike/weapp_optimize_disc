...
+function ($) { "use strict";
  ...
  let Button = function (element, options) {
...
+function ($) { "use strict";
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    let e = $.Event('slide.bs.carousel', { relatedTarget: $next[0], direction: direction })
    ...
    if (this.$indicators.length) {
      ...
      this.$element.one('slid', function () {
        let $nextIndicator = $(that.$indicators.children()[that.getActiveIndex()])
...
+function ($) { "use strict";
  ...
  Collapse.prototype.show = function () {
    ...
    let actives = this.$parent && this.$parent.find('> .panel > .in')

    if (actives && actives.length) {
      let hasData = actives.data('bs.collapse')
...
+function ($) { "use strict";
  ...
  let Dropdown = function (element) {
    let $el = $(element).on('click.bs.dropdown', this.toggle)
  }
  ...
  Dropdown.prototype.keydown = function (e) {
    ...
    let $items = $('[role=menu] li:not(.divider):visible a', $parent)
...
+function ($) { "use strict";
  ...
  $(document).on('click.bs.modal.data-api', '[data-toggle="modal"]', function (e) {
    ...
    let option  = $target.data('modal') ? 'toggle' : $.extend({ remote: !/#/.test(href) && href }, $target.data(), $this.data())
...
+function ($) { "use strict";
  ...
  Tooltip.prototype.init = function (type, element, options) {
    ...
    for (let i = triggers.length; i--;) {
      ...
      if (trigger == 'click') {
        ...
        let eventIn  = trigger == 'hover' ? 'mouseenter' : 'focus'
        let eventOut = trigger == 'hover' ? 'mouseleave' : 'blur'
  ...
  Tooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      if (autoPlace) {
        ...
        let parentWidth  = this.options.container == 'body' ? window.innerWidth  : $parent.outerWidth()
        let parentHeight = this.options.container == 'body' ? window.innerHeight : $parent.outerHeight()
        let parentLeft   = this.options.container == 'body' ? 0 : $parent.offset().left
  ...
  Tooltip.prototype.toggle = function (e) {
    let self = e ? $(e.currentTarget)[this.type](this.getDelegateOptions()).data('bs.' + this.type) : this
...
+function ($) { "use strict";
  ...
  function ScrollSpy(element, options) {
    let href
    let process  = $.proxy(this.process, this)
  ...
  ScrollSpy.prototype.refresh = function () {
    let offsetMethod = this.$element[0] == window ? 'offset' : 'position'

    this.offsets = $([])
    this.targets = $([])

    let self     = this
    let $targets = this.$body
      .find(this.selector)
      .map(function () {
        let $el   = $(this)
        let href  = $el.data('target') || $el.attr('href')
        let $href = /^#\w/.test(href) && $(href)
  ...
  ScrollSpy.prototype.process = function () {
    ...
    let scrollHeight = this.$scrollElement[0].scrollHeight || this.$body[0].scrollHeight
    let maxScroll    = scrollHeight - this.$scrollElement.height()
...
+function ($) { "use strict";
  ...
  Affix.prototype.checkPosition = function () {
    ...
    let affix = this.unpin   != null && (scrollTop + this.unpin <= position.top) ? false :
