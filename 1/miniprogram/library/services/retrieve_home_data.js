...
export default function () {
  const fetchStartigTime = new Date().getTime()
  ...
  const preFetchDataParallelCmd = new ParallelCommand([fetchData1Cmd, fetchData2Cmd, fetchData3Cmd, requestCmd], true)
    .onComplete(() => {
      ...
      // 演示没速监控数据上报
      const ms = new Date().getTime() - fetchStartigTime
      if (wx.canIUse("reportPerformance")) wx.reportPerformance(1001, ms)
      console.log(`监控上传：拉取首页数据耗时${ms}ms`)
    })
  // 代替原单一方式的数据拉取
