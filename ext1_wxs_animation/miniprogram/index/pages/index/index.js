...
Page({
  ...
  onTapRecycleItem: debounce(function(e) {
    ...
    const item = this.allList[index]
    const id = item.id 
    console.log(`点击了列表项${id}:${item.title}`);

    // 更新目标数据，使用forceUpdate
    // item.title += "+"
    // const recycleList = this.data.recycleList
    // const itemIndexInRecycleList = recycleList.findIndex(item => item.id === id)
    // this.setData({
    //   [recycleList[itemIndexInRecycleList].title]: item.title,
    // })
    // this.ctx.forceUpdate()

    // 更新目标数据，使用update
    item.title += "+"
    this.ctx.update(index, [item])
  }),
  ...
  dealWithListData(data) {
    ...
    this.allList = this.allList.concat(newList)
  ...
  onReady() {
    ...
    // 主页中从后端加载导航列表
    ;(async () => {
      const res1 = await promisify(wx.getStorage)({
        key: "navs"
      }).catch(console.log)
      if (res1 && res1.errMsg === "getStorage:ok") {
        const navs = res1.data
        console.log("已取到缓存的导航数据");
        this.setData({
